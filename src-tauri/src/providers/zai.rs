...
impl ZaiProvider {
    ...
    async fn fetch_usage(&self) -> Result<UsageSnapshot, anyhow::Error> {
        let api_key = self.resolve_api_token()
            .ok_or_else(|| anyhow::anyhow!("z.ai API token not found. Set Z_AI_API_KEY environment variable."))?;
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow::anyhow!("z.ai API error: HTTP {} - {}", status, body));
        }
        ...
        let data = api_response.data.ok_or_else(|| anyhow::anyhow!("z.ai API returned no data"))?;
    ...
    fn convert_response(&self, data: ZaiQuotaData) -> UsageSnapshot {
        ...
        let plan = data.plan_name.or(data.plan).or(data.plan_type).or(data.package_name);
    ...
    fn compute_used_percent(&self, limit: &ZaiLimitRaw) -> f64 {
        ...
        if limit.usage > 0 {
            ...
            if total > 0 {
                ...
                let used = limit.current_value.max(used_from_remaining).min(total).max(0);
    ...
    fn window_label(&self, limit: &ZaiLimitRaw) -> Option<String> {
        ...
        let suffix = if limit.number == 1 { unit_label } else { &format!("{}s", unit_label) };
